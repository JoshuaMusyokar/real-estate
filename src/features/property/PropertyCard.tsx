...
export const PropertyCard: FC<PropertyCardProps> = ({
  ...
  const canReview = can("property.approve"); // admin review action
  const canFeature = can("property.feature"); // feature/unfeature toggle

  // Whether the three-dot menu has anything to show
  ...
  return (
    <Card className="group overflow-hidden hover:shadow-xl hover:border-gray-300 transition-all duration-300">
      ...
      <div className="relative h-32 sm:h-40 md:h-48 lg:h-56 bg-gradient-to-br from-gray-100 to-gray-200 overflow-hidden">
        ...
        {/* Three-dot menu — only rendered when at least one action is available */}
        {hasAnyMenuAction && (
                  ...
                  {(canReview || canFeature) && (
                    <>
                      ...
                      {canFeature && (
                        ...
                        >
                          <Star
                            className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${property.featured ? "fill-current" : ""}`}
                          />
      ...
      <CardFooter className="p-2.5 sm:p-3 md:p-4 lg:px-5 lg:py-4">
        {/* Mobile view button — always shown */}
        <button
