...
export default function SignUpForm() {
  ...
  const [errors, setErrors] = useState<
    Partial<RegisterRequest & { terms: string }>
  >({});
  ...
  const validateForm = (): boolean => {
    const newErrors: Partial<RegisterRequest & { terms: string }> = {};
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await register({ ...formData, role: "BUYER" });
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-5">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <label
                    htmlFor="terms"
                    className="block font-normal text-gray-500 cursor-pointer dark:text-gray-400"
                  >
